...
table_demographics <- baseline_data %>% 
    ...
    mutate(sweat_chlorine_level = case_when(sweat_chlorine_level == "<40" ~ "<40",
                                            ...
                                            sweat_chlorine_level == "Missing" ~ "Missing",
                                            sweat_chlorine_level == "CFTR <40" ~ "CFTR <40",
                                            sweat_chlorine_level == "CFTR 40-<60" ~ "CFTR 40-<60",
                                            sweat_chlorine_level == "CFTR >=60" ~ "CFTR ≥60",
                                            sweat_chlorine_level == "CFTR Missing" ~ "CFTR Missing",
                                            sweat_chlorine_level == "Enrolment before Aug2022" ~ "Enrolment before Aug2022")) %>%
    mutate(sweat_chlorine_level = factor(sweat_chlorine_level, levels = c("<40", "40-<60", "≥60", "Missing",
                                                                          "CFTR <40", "CFTR 40-<60", "CFTR ≥60", "CFTR Missing",
                                                                          "Enrolment before Aug2022"))) %>%
    ...
    label=list(
      ...
      sex~"Sex", 
      genotype~"Genotype", 
      sweat_chlorine_level~"Sweat Chlorine Level in mmol/L",
...
``` {r}
table_demographics <- baseline_data %>% 
  select(sweat_chlorine_level)%>%
  tbl_summary()

sweat_chlorine_recodes <- 
  list(
    "People not on CFTR modulators" = c("<40", ">=60", "40-<60", "Missing"),
    "People on CFTR modulators" = c("CFTR <40", "CFTR >=60", "CFTR 40-<60", "CFTR Missing"),
    "Enrolment before Aug2022" = "Enrolment before Aug2022"
  )

tbl1 <- baseline_data %>% 
  mutate(
    new_sweat = forcats::fct_collapse(baseline_data$sweat_chlorine_level, !!!sweat_chlorine_recodes)
  ) %>%  
  select(new_sweat) %>% 
  tbl_summary()

as_kable_extra(tbl1, booktabs = TRUE, escape = FALSE, linesep = "") %>%
kable_styling(latex_options = "scale_down")
```
